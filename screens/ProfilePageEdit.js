import { Button, StyleSheet, Text, View, Pressable, Image, TextInput, SafeAreaView, ImageBackground} from 'react-native';
...
import {EditPicButton, NameInfo, ContinueButton, BackButton, PasswordInfo, EmailInfo, InstructionButton} from '../components/buttons';
import profileStyles from '../styles/profileStyles';
import profileEditStyles from '../styles/profileEditStyles';
import {useFonts,Poppins_400Regular} from '@expo-google-fonts/poppins';
import {getStorage, ref, uploadBytes,getDownloadURL, getMetadata, uploadBytesResumable} from 'firebase/storage';
import * as ImagePicker from 'expo-image-picker';
import { getDatabase, push, set, onValue } from "firebase/database";
import SearchableDropdown from 'react-native-searchable-dropdown';


const ProfilePageEdit = ({route, navigation}) => {
  ...
  //Get the current profile pic of the user.
  useEffect(() => {
  ...
  //change the description to what the user typed in textinput
  ...
  //change user year to what the user picked
  ...
  //change user major to what the user picked
  const changeMajor = (newMajor) =>{
    setUserMajor(newMajor) 
    const docRef = doc(usersDatabase, "users", email);
    updateDoc(docRef, {
      major: newMajor,
    });  
  }
  //Open the image picker, let the user choose an image, and upload that image to the backend.
  ...
  return (
    <View style={profileStyles.container}>
      <View>
      <BackButton onPress={() => navigation.navigate('Profile Page', {email:email})}></BackButton>
      </View>
      <View style={profileEditStyles.profilePicView}>
        {<ImageBackground source={{uri:profilePic}} resizeMode="cover" style={profileEditStyles.profilePic}>
        </ImageBackground>}
      </View>
      <View style={profileEditStyles.editPic}>
        <EditPicButton onPress={chooseImage}></EditPicButton>
      </View>
      <View style = {profileEditStyles.editDescription}>
        <Text style={profileEditStyles.titles}>Description</Text>
        <TextInput style={profileEditStyles.editBox} onChangeText={(newDescription) => changeDescription(newDescription)} maxLength={200}></TextInput>
      </View>
      <View style = {profileEditStyles.editDescription}>
        <Text style={profileEditStyles.titles}> Year</Text>
        <SelectList 
        // setSelected={(val) => setUserYear(val)}
        setSelected = {(val) => changeYear(val)} 
        data={years} 
        save="value"
        boxStyles = {profileEditStyles.editBox}
        />
      </View>
      <View style = {profileEditStyles.editDescription}>
        <Text style={profileEditStyles.titles}> Major </Text>
        <SelectList 
        setSelected={(val) => changeMajor(val)} 
        data={majors} 
        save="value"
        boxStyles = {profileEditStyles.editBox}
        />
